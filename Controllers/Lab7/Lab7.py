...
def compute_potential_field(robot_x, robot_y, robot_yaw, goal_x, goal_y, lidar_ranges, lidar_fov):
    ...
    U_att = k_att*(q_robot - q_goal)
    
    U_rep = -k_rep*(q_robot - q_obs)/(np.linalg.norm(q_robot -q_obs)**4)
    
    
    # front = np.percentile(ranges[235:266], 20)
    # front_right = np.percentile(ranges[285:340], 20)
    # right_side = np.median(ranges[360:430])
