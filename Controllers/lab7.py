...
def compute_potential_field(robot_x, robot_y, robot_yaw, goal_x, goal_y, lidar_ranges, lidar_fov):
    ...
    # Nlidar = len(lidar_ranges)
    # delta_theta_lidar = np.deg2rad(lidar_fov)/Nlidar
    # theta_lidar = 
    # k_att = 10 # attractive gain
    # k_rep = 5 # repelling gain
    # rho0 = np.array([0.5, 0.5]) # max distance needed for an object to be considered for repelling
    # q_goal = np.array([goal_x, goal_y])
    # q_robot = np.array([robot_x, robot_y])
    # U_att = 0.5*k_att*(q_robot - q_goal)**2
    # if rho <= rho0:
    #     U_rep = -0.5*k_rep*(1/rho - 1/rho0)**2
    # else:
    #     U_rep = 0
    # U_art = U_att + U_rep
    # base_speed = U_art[0]
    # turn = U_art[1]            
    # vL = clamp(base_speed - turn, -MAX_WHEEL_SPEED, MAX_WHEEL_SPEED)
    # vR = clamp(base_speed + turn, -MAX_WHEEL_SPEED, MAX_WHEEL_SPEED)
...
def load_environment_map():
    ...
    raise NotImplementedError("TODO 7: Implement load_environment_map()")


def navigate_to_goal(pr2, goal_x, goal_y, goal_yaw, env_map):
    while PR2Controller.robot.step(TIME_STEP) != -1:
        robot_x, robot_y, robot_yaw = pr2
        compute_potential_field(robot_x, robot_y, robot_yaw, goal_x, goal_y, lidar_ranges, lidar_fov)
